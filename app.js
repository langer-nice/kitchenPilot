const appState = {
  ...
  lastConsumedCookingSkipTimerAt: 0,
...
function hasSkippableActiveTimerForCurrentCookingStep() {
  ...
  if (appState.timerSkippedStepIndex === appState.cookingIndex) {
    return false;
  }

  if (appState.timerStatus === "completed" || appState.timerStatus === "skipped") {
    return false;
  }

  if (appState.timerStatus === "running" || appState.timerStatus === "paused") {
    return true;
  }

  return Number.isFinite(appState.activeTimerSeconds) && appState.activeTimerSeconds > 0;
}
...
function consumeCookingSkipTimerCommand(commandText, timing = {}) {
  const consumedAt = Number.isFinite(timing.matchedAt)
    ? timing.matchedAt
    : (Number.isFinite(timing.transcriptReceivedAt) ? timing.transcriptReceivedAt : getVoiceTimestamp());
  appState.lastConsumedCookingSkipTimerAt = consumedAt;
...
function clearCookingLiveCommandStateAfterStepTransition(source = "unknown", nextCookingIndex = appState.cookingIndex) {
  const now = getVoiceTimestamp();
  const transcriptToIgnore = normalizeVoiceCommandText(
    appState.voiceLastTranscript || appState.voiceLastAcceptedCommandTranscript || ""
  );
  const currentSpeechSessionId = Number(appState.voiceUserSpeechSessionId || 0);

  if (transcriptToIgnore && currentSpeechSessionId) {
    appState.ignoredCookingLiveCommandSessionId = currentSpeechSessionId;
    appState.ignoredCookingLiveCommandTranscript = transcriptToIgnore;
    appState.ignoredCookingLiveCommandUntil = now + 1200;
  } else {
  ...
  recordCookingDebugEvent("cooking-live-command-cleared-source", {
    source,
    nextCookingIndex,
    transcript: appState.voiceLastTranscript || "",
    matchedCommand: appState.voiceLastMatchedCommand || "",
    lastAction: appState.voiceLastAction || "",
    ignoredTranscript: appState.ignoredCookingLiveCommandTranscript || "",
    ignoredSessionId: appState.ignoredCookingLiveCommandSessionId || 0
  });
  ...
  recordCookingDebugEvent("cooking-live-command-state-reset", {
    source,
    nextCookingIndex,
    ignoredTranscript: appState.ignoredCookingLiveCommandTranscript || "",
    ignoredSessionId: appState.ignoredCookingLiveCommandSessionId || 0,
    ignoredUntil: appState.ignoredCookingLiveCommandUntil || 0
  });
  recordCookingDebugEvent("cooking-live-command-cleared-after-step-transition", {
    source,
    nextCookingIndex
  });
}
...
function shouldIgnoreCookingLiveCommandAfterStepTransition(commandText, timing = {}) {
  ...
  const ignoredTranscript = appState.ignoredCookingLiveCommandTranscript || "";
  const ignoredSessionId = Number(appState.ignoredCookingLiveCommandSessionId || 0);
  const ignoredUntil = Number(appState.ignoredCookingLiveCommandUntil || 0);
  if (!ignoredTranscript || !ignoredSessionId || !ignoredUntil) {
    return false;
  }
  ...
  if (
    normalizedTranscript === ignoredTranscript &&
    currentSpeechSessionId === ignoredSessionId &&
    now <= ignoredUntil
  ) {
    appState.voiceHeard = "";
    ...
    appState.voiceLastAction = "";
    recordCookingDebugEvent("cooking-live-command-state-reset", {
      source: "post-step-transition-ignore",
      ignoredTranscript,
      ignoredSessionId,
      ignoredUntil
    });
  ...
  if (currentSpeechSessionId !== ignoredSessionId || now > ignoredUntil) {
    appState.ignoredCookingLiveCommandSessionId = 0;
    appState.ignoredCookingLiveCommandTranscript = "";
    appState.ignoredCookingLiveCommandUntil = 0;
  }
...
function shouldAllowCookingSkipTimerCommand(commandText, timing = {}) {
  ...
  if (!hasSkippableActiveTimerForCurrentCookingStep()) {
    recordCookingDebugEvent("cooking-step-advance-blocked", {
      source: "voice-skip-timer",
  ...
  if (
    ...
    recordCookingDebugEvent("cooking-skip-timer-rejected-as-already-used", {
      ...
      normalizedTranscript,
      speechSessionId: currentSpeechSessionId,
      lastConsumedAt: appState.lastConsumedCookingSkipTimerAt || null,
      lastConsumedStepIndex: appState.lastConsumedCookingSkipTimerStepIndex
    });
    recordCookingDebugEvent("cooking-step-advance-blocked", {
      source: "voice-skip-timer",
      reason: "already-used",
  ...
  if (
    normalizedTranscript === appState.lastConsumedCookingSkipTimerTranscript &&
    Number(appState.lastConsumedCookingSkipTimerAt || 0) &&
...
function goToNextCookingStep() {
  ...
  const advanceSource = appState.lastCookingNextTriggerSource || "unknown";
  ...
  if (appState.cookingIndex < appState.recipe.cookingSteps.length - 1) {
    ...
    appState.timerSkippedStepIndex = null;
    clearCookingLiveCommandStateAfterStepTransition(advanceSource, appState.cookingIndex);
    renderCooking();
  } else {
...
function skipTimerAndAdvance(source = "timer-skip-advance") {
  if (!hasSkippableActiveTimerForCurrentCookingStep()) {
    recordCookingDebugEvent("cooking-step-advance-blocked", {
      source,
      reason: "no-active-timer",
  ...
  const skippedStepIndex = appState.cookingIndex;
  skipActiveTimer();
  recordCookingDebugEvent("cooking-skip-timer-applied-to-step", {
    source,
  ...
  recordCookingDebugEvent("cooking-step-advance-triggered-by-skip-timer", {
    source,
    cookingIndex: skippedStepIndex,
    targetCookingIndex: skippedStepIndex + 1
  });
  goToNextCookingStep();
}
...
function matchCommand(commandKey) {
  ...
  if (command === "skip timer") {
    if (!shouldAllowCookingSkipTimerCommand(commandText, timing)) {
      return;
    }
    matchCommand("skip_timer");
    consumeCookingSkipTimerCommand(commandText, timing);
    setVoiceCommandLock("timer:skip");
    runVoiceAction("skip-timer", "Skip Timer", () => {
      skipTimerAndAdvance("voice-skip-timer");
    }, 0, timing);
    return;
  }
  ...
  if (timerInteractionActive) {
    primaryRow.append(
      ...
      createButton("Skip Timer", "", () => {
        skipTimerAndAdvance("click-skip-timer");
      }, "skip-timer")
    );
  } else {
    primaryRow.append(
